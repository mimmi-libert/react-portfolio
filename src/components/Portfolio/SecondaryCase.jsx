...
function SecondaryCase({ id, category, organization, image, status, index }) {
  const { t } = useLanguage();
  const isUnderConstruction = status === "under-construction";
  const caseId = `secondary-case-${index}`;

  // Apply translations here - same pattern as MainCase
  const translatedCaseData = {
    id,
    category,
    organization: t(`portfolio.cases.${id}.organization`),
    description: t(`portfolio.cases.${id}.description`),
    image,
    status,
  };
  ...
  return (
    <article className="relative grid gap-y-2xs">
      <LangLink
      ...
      <div className="cases__secondary__content grid gap-2 md:gap-3">
        ...
        >
          <h3
            id={`${caseId}-heading`}
            className="text-white hover:text-white-transparent cursor-pointer transition-colors"
          >
