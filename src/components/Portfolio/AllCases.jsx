...
import { getAllCases } from "./portfolio-data";
  ...
  if (status === "under-construction") {
    return (
      <svg
        ...
        aria-hidden="true"
        role="img"
        className="w-14 h-14 lg:w-full lg:h-full"
      >
...
function AllCases() {
  ...
  // Apply translations here - same pattern as MainCase
  const translatedCases = allCases.map((caseItem) => ({
    ...caseItem,
    organization: t(`portfolio.cases.${caseItem.id}.organization`),
    description: t(`portfolio.cases.${caseItem.id}.description`),
    fullDescription: t(`portfolio.cases.${caseItem.id}.fullDescription`),
  }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-sm lg:gap-x-lg gap-y-sm lg:gap-y-md">
          {translatedCases.map((caseItem) => {
            const isCaseUnderConstruction =
              caseItem.status === "under-construction";

            // Create category labels array for alt text
            const categoryLabels = Object.keys(caseItem.category).map((cat) =>
              t(`portfolio.categories.${cat}`)
            );

            return (
              <article key={caseItem.id} className="relative">
                ...
                <div className="cases__secondary__content pt-2xs lg:pt-xs">
                  <LangLink
                    to={`/case/${caseItem.id}`}
                    className="block focus-visible:outline-2 focus-visible:outline-white focus-visible:outline-offset-4"
                  >
                    <h3
                      id={`${caseItem.id}-heading`}
                      className="text-white hover:text-white-transparent pb-2 lg:pb-3 text-md cursor-pointer"
                    >
                  ...
                  {isCaseUnderConstruction && (
                    <div
                      id={`${caseItem.id}-construction-notice`}
                      className="sr-only"
                      aria-live="polite"
                    >
